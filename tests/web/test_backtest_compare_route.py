...
def _compare_frame(rows: int = 288, symbol: str = "BTC/USDT", freq: str = "5min") -> pd.DataFrame:
    idx = pd.date_range("2025-01-01", periods=rows, freq=freq)
...
def _fake_attach_backtest_enrichment_if_needed(df: pd.DataFrame):
    enriched = df.copy()
...
def test_compare_optimization_plan_caps_large_history_windows():
    from web.api import backtest as backtest_api
...
def test_compare_backtests_limits_preoptimization_scope_for_large_history(monkeypatch):
    ...
    df = _compare_frame(rows=22883, freq="1h")
    strategies = [
        "ADXTrendStrategy",
        "BollingerBandsStrategy",
