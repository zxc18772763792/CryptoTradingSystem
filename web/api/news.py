...
async def build_latest_feed(
    ...
    events_by_title: Dict[str, List[Dict[str, Any]]] = {}
    for event in events:
        evidence = event.get("evidence") or {}
        ...
        if title_key:
            events_by_title.setdefault(title_key, []).append(event)
    ...
    for raw in raw_items:
        ...
        if not matched_events:
            matched_events = events_by_title.get(_canonical_title(raw.get("title"))) or []
...
async def pull_and_store_news(cfg: Dict[str, Any], payload: PullNowRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        # If keyword filtering is too strict, top up with latest raw headlines
        # to keep dashboard freshness and avoid stale-only feeds.
...
async def latest(
    ...
    cfg = _get_cfg(request)
    feed = await build_latest_feed(cfg=cfg, symbol=symbol, hours=hours, limit=limit, summarize=summarize)
