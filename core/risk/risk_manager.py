...
class RiskManager:
    ...
    def update_equity(self, total_usd: float) -> None:
    ...
    def _evaluate_daily_stop(self) -> None:
        ...
        breach_ratio = daily_pnl_ratio <= -abs(self.max_daily_loss_ratio)
        breach_usd = self.max_daily_loss_usd > 0 and daily_pnl_usd <= -abs(self.max_daily_loss_usd)

        if (breach_ratio or breach_usd) and not self._trading_halted:
            self._trading_halted = True
            self._halt_reason = (
                f"触发日内止损，日内收益 {daily_pnl_ratio * 100:.2f}% "
                f"({daily_pnl_usd:.2f} USDT)"
            )
            self._add_alert(
                title="触发日内风控熔断",
    ...
    def get_risk_metrics(self) -> RiskMetrics:
        ...
        daily_pnl_ratio = (daily_pnl_usd / day_start) if day_start > 0 else 0.0
    ...
    def get_risk_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "equity": {
                "current": round(float(self._current_equity or 0.0), 4),
                "day_start": round(float(self._day_start_equity or 0.0), 4),
                ...
                "pnl_scope_note": "daily_pnl_usd为今日权益变化；current_unrealized_pnl_usd为当前持仓浮动盈亏",
            },
            "limits": {
